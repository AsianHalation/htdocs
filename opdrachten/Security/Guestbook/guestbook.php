...
<div class="body-container">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $color = $_POST['color'];
        //SQL Injection preventie door prepare and execute functies - https://phpdelusions.net/pdo - protection from sql injections
        $sql = "INSERT INTO `entries`(`email`, `color`, `admin`, `text`) VALUES (?, ?, ?, ?)";
        $conn->prepare($sql)->execute([$email, $color, $admin, $text]);
    }

    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        echo "dit is een email";
    } else {
        echo "dit is geen mail";
    }
